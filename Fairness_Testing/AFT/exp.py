...
def exp(dataset_name, model_name, protected_pair, method, runtime, repeat, show_logging, res_logging=False, repeat_label=None, start_label=0):
    ...
    dataset_path = os.path.join(base_dir, "Datasets", f"{dataset_csv}.csv")
    data_range, df = BlackBoxModel.create_data_range_from_csv(dataset_path)
    model_path = os.path.join(base_dir, "FairnessTestCases", f"{model_name}{dataset_name}.joblib")
...
def para_exp_main(runtime, repeat, repeat_run_together=True):
    check_models = ["LogReg", "RanForest", "DecTree", "MLP", "SVM"]
