    ...
    for times in range(repeat_times):
        ...
        def evaluate_disc(inp):
            ...
            if abs(out0 - out1) > threshold and tuple(map(tuple, inp0)) not in disc_inputs:
                ...
                #if len(disc_inputs_list) >= dataset_num:         #modified
                #    search_finished = True                       #modified
        ...
        output_dir = os.path.join(project_root, "Distance", "result", algorithm.lower()+"_noncut", "{}-{}".format(dataset_name, sensitive_name), classifier)
